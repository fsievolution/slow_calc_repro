...
defmodule AshSql.Expr do
  ...
  def dynamic_expr(query, expression, bindings, embedded?, type, acc) do
    ...
    result = do_dynamic_expr(query, expression, bindings, embedded?, acc, type)
    end_time = System.monotonic_time(:microsecond)

    if end_time - start_time > 1000 do  # Only log if >1ms
      IO.puts("      [TRACE] dynamic_expr took #{end_time - start_time}μs for expr type: #{inspect(expression.__struct__)}")
    end
  ...
  defp do_dynamic_expr(query, expr, bindings, embedded?, acc, type \\ nil) do
    ...
    # Create a short hash of the expr to detect if same expression is processed multiple times
    expr_id = if is_struct(expr), do: "#{expr.__struct__ |> Module.split() |> List.last()}_#{:erlang.phash2(expr, 1000)}", else: "literal"

    result = case bindings.sql_behaviour.expr(query, expr, bindings, embedded?, acc, type) do
      {:ok, expr, acc} -> {expr, acc}
      {:error, error} -> {:error, error}
      :error ->
        before_default = System.monotonic_time(:microsecond)
        result = default_dynamic_expr(query, expr, bindings, embedded?, acc, type)
        after_default = System.monotonic_time(:microsecond)
        if after_default - before_default > 5000 do
          IO.puts("      [TRACE]   - default_dynamic_expr for #{expr_id}: #{after_default - before_default}μs")
        end
        result
    end

    end_time = System.monotonic_time(:microsecond)
    if end_time - start_time > 10000 do  # Only log if >10ms
      IO.puts("      [TRACE] do_dynamic_expr took #{end_time - start_time}μs for #{expr_id} (type: #{inspect(expr.__struct__)})")
    end
