...
defmodule Ash.Actions.Read.Calculations do
  ...
  defp attach_calculation_results(calculation, records, values) do
    ...
    IO.puts("\n[TRACE #{start_time}] attach_calculation_results START (list variant)")
    IO.puts("  calculation.name: #{inspect(calculation.name)}")
    IO.puts("  num_records: #{length(records)}")
    IO.puts("  num_values: #{length(values)}")

    result = if calculation.load do
    ...
    IO.puts("[TRACE] attach_calculation_results END (#{end_time - start_time}μs)\n")
    result
  end

  defp run_calculation(calculation, ash_query, records) do
    start_time = System.monotonic_time(:microsecond)
    IO.puts("\n[TRACE #{start_time}] run_calculation START")
    IO.puts("  calculation.name: #{inspect(calculation.name)}")
    IO.puts("  calculation.module: #{inspect(calculation.module)}")
    IO.puts("  num_records: #{length(records)}")
    ...
    before_transient = System.monotonic_time(:microsecond)
    IO.puts("  [#{before_transient - start_time}μs] Calling apply_transient_calculation_values...")

    records
    |> apply_transient_calculation_values(calculation, ash_query, [])
    |> tap(fn _ ->
      after_transient = System.monotonic_time(:microsecond)
      IO.puts("  [#{after_transient - start_time}μs] Calling run_calculate...")
    end)
    |> run_calculate(
      calculation.module,
      opts,
    ...
    |> case do
      :unknown ->
        ...
        IO.puts("  [#{now - start_time}μs] Result: :unknown")
      ...
      result ->
        now = System.monotonic_time(:microsecond)
        IO.puts("  [#{now - start_time}μs] Result returned, length: #{if is_list(result), do: length(result), else: "not a list"}")
        result
    end
    |> tap(fn _ ->
      end_time = System.monotonic_time(:microsecond)
      IO.puts("[TRACE] run_calculation END (total: #{end_time - start_time}μs)\n")
    end)
  end
  ...
  defp apply_transient_calculation_values(records, calculation, ash_query, path) do
    start_time = System.monotonic_time(:microsecond)
    IO.puts("  [TRACE #{start_time}] apply_transient_calculation_values START")
    IO.puts("    num_records: #{length(records)}")

    rewrites = ash_query
    |> get_all_rewrites(calculation, path)

    after_rewrites = System.monotonic_time(:microsecond)
    IO.puts("    [#{after_rewrites - start_time}μs] num_rewrites: #{length(rewrites)}")

    result = rewrite(rewrites, records)

    end_time = System.monotonic_time(:microsecond)
    IO.puts("  [TRACE] apply_transient_calculation_values END (#{end_time - start_time}μs)")
    result
  end
  ...
  def rewrite(rewrites, [%resource{} | _] = records) do
    start_time = System.monotonic_time(:microsecond)
    IO.puts("    [TRACE #{start_time}] rewrite START")
    IO.puts("      num_rewrites: #{length(rewrites)}")
    IO.puts("      num_records: #{length(records)}")
    ...
    |> Enum.reduce(records, fn
      {{:cleanup_field_auth, further_load}, idx}, records ->
        rewrite_start = System.monotonic_time(:microsecond)
        IO.puts("      [#{rewrite_start - start_time}μs] [rewrite #{idx}] cleanup_field_auth")
        query =
          resource
      ...
      {{{path, data, calc_name, calc_load}, source}, idx}, records when path != [] ->
        rewrite_start = System.monotonic_time(:microsecond)
        IO.puts("      [#{rewrite_start - start_time}μs] [rewrite #{idx}] path rewrite (path length: #{length(path)})")
        result = rewrite_at_path(records, {{path, data, calc_name, calc_load}, source})
        rewrite_end = System.monotonic_time(:microsecond)
        IO.puts("      [#{rewrite_end - start_time}μs] [rewrite #{idx}] path rewrite done (took #{rewrite_end - rewrite_start}μs)")
        result

      {{{_, {:calc, to_name, to_load}, _, _}, source}, idx}, records ->
        rewrite_start = System.monotonic_time(:microsecond)
        IO.puts("      [#{rewrite_start - start_time}μs] [rewrite #{idx}] :calc rewrite, mapping #{length(records)} records")
        result = if to_load do
          Enum.map(records, fn record ->
            case record do
              %Ash.NotLoaded{} ->
                record
        ...
        rewrite_end = System.monotonic_time(:microsecond)
        IO.puts("      [#{rewrite_end - start_time}μs] [rewrite #{idx}] :calc done (took #{rewrite_end - rewrite_start}μs)")
        result

      {{{_, {:agg, to_name, to_load}, _, _}, source}, idx}, records ->
        rewrite_start = System.monotonic_time(:microsecond)
        IO.puts("      [#{rewrite_start - start_time}μs] [rewrite #{idx}] :agg rewrite, mapping #{length(records)} records")
        result = if to_load do
          Enum.map(records, fn record ->
            case record do
              %Ash.NotLoaded{} ->
        ...
        rewrite_end = System.monotonic_time(:microsecond)
        IO.puts("      [#{rewrite_end - start_time}μs] [rewrite #{idx}] :agg done (took #{rewrite_end - rewrite_start}μs)")
        result

      {{{path, {:rel, rel_name}, _, _}, source}, idx}, records ->
        rewrite_start = System.monotonic_time(:microsecond)
        IO.puts("      [#{rewrite_start - start_time}μs] [rewrite #{idx}] :rel rewrite")
        result = records
        |> update_at_path(path, fn record ->
          Map.put(record, rel_name, Map.get(record.calculations, source))
        end)
        rewrite_end = System.monotonic_time(:microsecond)
        IO.puts("      [#{rewrite_end - start_time}μs] [rewrite #{idx}] :rel done (took #{rewrite_end - rewrite_start}μs)")
        result
    end)

    end_time = System.monotonic_time(:microsecond)
    IO.puts("    [TRACE] rewrite END (#{end_time - start_time}μs)")
    result
  end
