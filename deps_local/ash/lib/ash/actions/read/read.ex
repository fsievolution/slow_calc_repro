...
defmodule Ash.Actions.Read do
  ...
  defp do_run(query, action, opts) do
    ...
    before_split = System.monotonic_time(:microsecond)
    IO.puts("[TRACE] [#{before_split - start_time}μs] Calling split_and_load_calculations...")
    ...
    case Ash.Actions.Read.Calculations.split_and_load_calculations(
      ...
      {:ok, calculations_in_query, calculations_at_runtime, query} ->
        after_split = System.monotonic_time(:microsecond)
        IO.puts("[TRACE] [#{after_split - start_time}μs] split_and_load_calculations done (#{after_split - before_split}μs)")
        IO.puts("[TRACE]   - calculations_in_query: #{length(calculations_in_query)}")
        IO.puts("[TRACE]   - calculations_at_runtime: #{length(calculations_at_runtime)}")
        ...
        if opts[:data_layer_query?] do
          ...
          try do
            before_read = System.monotonic_time(:microsecond)
            IO.puts("[TRACE] [#{before_read - start_time}μs] Calling do_read/load...")
            ...
            after_read = System.monotonic_time(:microsecond)
            IO.puts("[TRACE] [#{after_read - start_time}μs] do_read/load complete (#{after_read - before_read}μs)")
            ...
            with {:ok, data, count, calculations_at_runtime, calculations_in_query, new_query} <-
                 ...
                 _ = (
                   before_load_attrs1 = System.monotonic_time(:microsecond)
                   IO.puts("[TRACE] [#{before_load_attrs1 - start_time}μs] Calling load_through_attributes (1)...")
                 ),
                 {:ok, data} <-
                 ...
                 _ = (
                   after_load_attrs1 = System.monotonic_time(:microsecond)
                   IO.puts("[TRACE] [#{after_load_attrs1 - start_time}μs] load_through_attributes (1) done (#{after_load_attrs1 - before_load_attrs1}μs)")
                   IO.puts("[TRACE]   - num records: #{length(data)}")
                   IO.puts("[TRACE] [#{after_load_attrs1 - start_time}μs] Calling load_relationships...")
                 ),
                 {:ok, data} <-
                   load_relationships(data, query, opts),
                 _ = (
                   after_load_rels = System.monotonic_time(:microsecond)
                   IO.puts("[TRACE] [#{after_load_rels - start_time}μs] load_relationships done (#{after_load_rels - after_load_attrs1}μs)")
                   IO.puts("[TRACE] [#{after_load_rels - start_time}μs] Calling Calculations.run...")
                 ),
                 {:ok, data} <-
                 ...
                 _ = (
                   after_calc_run = System.monotonic_time(:microsecond)
                   IO.puts("[TRACE] [#{after_calc_run - start_time}μs] Calculations.run done (#{after_calc_run - after_load_rels}μs)")
                   IO.puts("[TRACE] [#{after_calc_run - start_time}μs] Calling load_through_attributes (2)...")
                 ),
                 {:ok, data} <-
                   load_through_attributes(
                     data,
              ...
              after_load_attrs2 = System.monotonic_time(:microsecond)
              IO.puts("[TRACE] [#{after_load_attrs2 - start_time}μs] load_through_attributes (2) done (#{after_load_attrs2 - after_calc_run}μs)")
              IO.puts("[TRACE] [#{after_load_attrs2 - start_time}μs] Final cleanup and formatting...")
              ...
              IO.puts("[TRACE #{end_time}] ===== do_run END (total: #{end_time - start_time}μs) =====\n")
  ...
  defp do_read(
    ...
    do_read_start = System.monotonic_time(:microsecond)
    IO.puts("  [TRACE #{do_read_start}] >> do_read START")

    with {:ok, %{valid?: true} = query} <- handle_multitenancy(query),
         query <- add_select_if_none_exists(query),
         pre_authorization_query <- query,
         _ = (
           after_pre_auth = System.monotonic_time(:microsecond)
           IO.puts("  [TRACE] [#{after_pre_auth - do_read_start}μs] authorize_query...")
         ),
      ...
      after_auth_sort = System.monotonic_time(:microsecond)
      IO.puts("  [TRACE] [#{after_auth_sort - do_read_start}μs] auth/sort done, run_before_transaction_hooks...")

      case run_before_transaction_hooks(query) do
        {:ok, query} ->
          after_hooks = System.monotonic_time(:microsecond)
          IO.puts("  [TRACE] [#{after_hooks - do_read_start}μs] hooks done (#{after_hooks - after_auth_sort}μs), maybe_in_transaction...")

          maybe_in_transaction(query, opts, fn notify_callback ->
            after_txn_start = System.monotonic_time(:microsecond)
            IO.puts("  [TRACE] [#{after_txn_start - do_read_start}μs] in transaction")
            ...
            with query_before_pagination <- query,
                 ...
                 _ = (
                   after_deselect = System.monotonic_time(:microsecond)
                   IO.puts("  [TRACE] [#{after_deselect - do_read_start}μs] deselect done, hydrate_calculations...")
                 ),
                 {:ok, data_layer_calculations} <-
                   hydrate_calculations(query, calculations_in_query),
                 _ = (
                   after_hydrate_calcs = System.monotonic_time(:microsecond)
                   IO.puts("  [TRACE] [#{after_hydrate_calcs - do_read_start}μs] hydrate_calculations done (#{after_hydrate_calcs - after_deselect}μs)")
                 ),
                 {:ok, query} <- hydrate_aggregates(query),
                 _ = (
                   after_hydrate_aggs = System.monotonic_time(:microsecond)
                   IO.puts("  [TRACE] [#{after_hydrate_aggs - do_read_start}μs] hydrate_aggregates done")
                 ),
                 {:ok, query} <-
                   hydrate_sort(
                     query,
                 ...
                 _ = (
                   after_hydrate_sort = System.monotonic_time(:microsecond)
                   IO.puts("  [TRACE] [#{after_hydrate_sort - do_read_start}μs] hydrate_sort done")
                 ),
                 {:ok, query} <-
                 ...
                 _ = (
                   before_rel_filters = System.monotonic_time(:microsecond)
                   IO.puts("  [TRACE] [#{before_rel_filters - do_read_start}μs] calling Ash.Filter.relationship_filters...")
                 ),
                 ...
                 _ = (
                   after_rel_filters = System.monotonic_time(:microsecond)
                   IO.puts("  [TRACE] [#{after_rel_filters - do_read_start}μs] relationship_filters done (#{after_rel_filters - before_rel_filters}μs)")
                 ),
                 ...
                 _ = (
                   after_before_action = System.monotonic_time(:microsecond)
                   IO.puts("  [TRACE] [#{after_before_action - do_read_start}μs] run_before_action done, fetch_count...")
                 ),
                 {:ok, count} <-
                   fetch_count(
                     query,
              ...
              after_fetch_count = System.monotonic_time(:microsecond)
              IO.puts("  [TRACE] [#{after_fetch_count - do_read_start}μs] fetch_count done (#{after_fetch_count - after_before_action}μs)")
              ensure_task_stopped(count, fn ->
                with {:ok, query} <- paginate(query, action, opts[:skip_pagination?]),
                     ...
                     _ = (
                       before_data_layer_query = System.monotonic_time(:microsecond)
                       IO.puts("  [TRACE] [#{before_data_layer_query - do_read_start}μs] calling Ash.Query.data_layer_query...")
                     ),
                     {:ok, data_layer_query} <-
                     ...
                     _ = (
                       after_data_layer_query = System.monotonic_time(:microsecond)
                       IO.puts("  [TRACE] [#{after_data_layer_query - do_read_start}μs] data_layer_query done (#{after_data_layer_query - before_data_layer_query}μs)")
                     ),
                     _ = (
                       before_run_query = System.monotonic_time(:microsecond)
                       IO.puts("  [TRACE] [#{before_run_query - do_read_start}μs] *** CALLING run_query (SQL) ***")
                     ),
                     {{:ok, results}, query} <-
                       run_query(
                     ...
                     _ = (
                       after_run_query = System.monotonic_time(:microsecond)
                       IO.puts("  [TRACE] [#{after_run_query - do_read_start}μs] *** run_query DONE (#{after_run_query - before_run_query}μs) ***")
                     ),
